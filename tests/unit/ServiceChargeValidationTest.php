...
class ServiceChargeValidationTest extends TestCase
{
    ...
    /**
     * Test that total phase cannot be applied to products.
     *
     * @return void
     */
    public function test_total_phase_cannot_be_applied_to_products(): void
    {
        $this->expectException(ValidationException::class);
        $this->expectExceptionMessage('Total phase service charges cannot be applied at the product (line-item) level');

        factory(ServiceCharge::class)->create([
            'calculation_phase' => Constants::SERVICE_CHARGE_CALCULATION_PHASE_TOTAL,
            'treatment_type' => Constants::SERVICE_CHARGE_TREATMENT_LINE_ITEM,
            'percentage' => 5.0,
            'taxable' => false, // Must be false for total phase
        ]);
    }
